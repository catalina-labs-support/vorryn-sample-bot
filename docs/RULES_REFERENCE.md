...
**Causeway free roads:** Playing Causeway (`scienceRoadBuilding`) sets `freeRoadsRemaining` to `min(2, roadsInSupply)` and opens a `roadBuildingPlace` pending decision. Each `buildRoad` action inside that pending decrements `freeRoadsRemaining`; the pending clears automatically when it reaches 0. The player may also submit `skipRoadBuilding` to forfeit remaining free roads. No resource cost is charged during the pending.
...
1. `domesticTradePropose` creates a `DomesticTradeResponse` pending targeting all opponents (or the named target). Each opponent in `allowedPlayerIds` must respond before the pending resolves.
2. Any opponent may play `domesticTradeAccept` (must hold the full `want` side at accept time — re-validated at accept time), `domesticTradeDecline` (no conditions), or — instead of accepting or declining — reply with a single counter-offer (`domesticTradeCounter`). A counter states new terms from the **proposer's perspective**: `offer` is what the proposer would give, `want` is what the countering responder would give (the responder must hold that `want` side at counter time). Countering consumes that responder's reply — one counter round, counters cannot themselves be countered.
3. Once every opponent has acted, the pending transitions to `DomesticTradeConfirm` if at least one opponent accepted **or** at least one recorded a counter; otherwise it resolves to null (proposal fully declined). Recorded counters ride along on the pending payload as a `counters` array of `{ responderId, offer, want }` entries and are preserved through any mix of later accepts and declines.
...
```json
{
  "offer": { "type": "<MaterialType>", "count": <rate> },
  "want":  { "type": "<MaterialType>", "count": 1 }
}
```

`offer.count` must equal the player's best applicable rate for `offer.type`. `want.count` is always 1. Offer and want types must differ. Both resources and commodities may be offered; both resources and commodities may be requested. Bank stock is checked: the requested type must have at least 1 in `bankResources` or `bankCommodities` (§6).
...
Grants up to 2 free road placements. Sets `player.freeRoadsRemaining = min(2, roadsInSupply)` and installs a `roadBuildingPlace` pending. Road placements then proceed exactly as paid `buildRoad` actions (each road must be legally connected when placed). If only 1 road remains in supply, only 1 free road is granted.

The player may decline remaining road placements via `skipRoadBuilding`.

**Pendings:** `roadBuildingPlace` (self; repeated until `freeRoadsRemaining` reaches 0 or player skips via `SkipRoadBuilding`).
...
If the removed road belonged to the actor and the actor has roads in supply, a free road placement is granted immediately: `freeRoadsRemaining = 1` and a `roadBuildingPlace` pending is installed.

**Pending:** `roadBuildingPlace` (self; only when the actor's own road was removed and supply > 0).
...
The 21 pending-decision types, their triggers, which player must answer, and the legal answer action(s):
...
| `roadBuildingPlace`       | Causeway card (`scienceRoadBuilding`, §14) or Diplomacy card (`politicsDiplomacy`, §14) when actor's own road is removed                 | The card player                                                                                              | `buildRoad` with `edgeId`; or `skipRoadBuilding` to forfeit remaining free roads                                                |
...
All 35 action types and the section(s) where they are documented:
...
| `domesticTradeAccept`        | §12                                                    |
| `domesticTradeDecline`       | §12                                                    |
| `domesticTradeConfirm`       | §12, §16                                               |
| `domesticTradeCounter`       | §12                                                    |
| `domesticTradeTakeCounter`   | §12                                                    |
