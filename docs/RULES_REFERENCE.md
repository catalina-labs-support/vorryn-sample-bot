...
| `weddingGiveCards`        | Betrothal card (`politicsBetrothal`, §14)                                                                                                                                 | Each opponent with more VPs than the actor and a non-empty hand (empty-handed opponents are skipped), in sequence (one at a time)    | `resolveOptionalCardEffect` with `cards` array of `{ type, count }` to give; must total exactly `requiredCount`                                                     |
