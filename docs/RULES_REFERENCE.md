...
3. The pending stays a `DomesticTradeResponse` throughout — there is no separate confirm-stage pending. The proposer may resolve a standing accept or counter **in place at any time** (an open auction: the proposer need not wait for every opponent to answer). If every opponent declines with no acceptance or counter standing, the pending resolves to null (proposal fully declined). Recorded counters ride along on the pending payload as a `counters` array of `{ responderId, offer, want }` entries and are preserved through any mix of later accepts and declines.
...
5. `domesticTradeCancel` withdraws the open proposal at any point while the `DomesticTradeResponse` pending is active. Only the current player may cancel.

An un-taken counter is not recorded as a decline.

Note: `domesticTradeConfirm`/`domesticTradeTakeCounter` are `ActionType`s only — there is no `DomesticTradeConfirm` pending type. The proposer is recorded as the `DomesticTradeResponse` pending's `actingPlayerId` and is deliberately **not** a member of `allowedPlayerIds` (which names the responders); the proposer resolves via those actions off that same pending.
...
| `domesticTradeResponse`   | `domesticTradePropose` (§12.1)                                                                                                                                            | Each targeted opponent may respond (accept/decline/counter); the proposer resolves a standing bid or cancels — an open auction, no separate confirm pending | Responders: `domesticTradeAccept`, `domesticTradeDecline`, `domesticTradeCounter`. Proposer: `domesticTradeConfirm` (optional `accepterId`), `domesticTradeTakeCounter` (`responderId`), or `domesticTradeCancel` (current player only) |
